...
def save_correspondences_pair_gt(pair):
    ...
    if gt_data is not None:
        correspondences = radar.build.radar_processor.get_radar_correspondences(
            pair['radar_path'], pair['radar_imgs'][0], pair['radar_imgs'][1], 1)
        correspondences.append(np.array(gt_data))
        np.save(correspondence_path + '/' + 'pair_%d_%d.npy' %
                (t1, t2), correspondences)
    else:
...
def create_pairs(seq):
    ...
    correspondence_path = root_path + '/correspondences/' + seq
    try:
